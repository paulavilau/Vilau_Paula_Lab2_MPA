...
namespace Vilau_Paula_Lab2.Controllers
{
    public class PublishersController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? id, int? bookID, int? customerId)
        {
            ...
            if (bookID != null)
            {
                ...
                Book book = viewModel.Books.Where(i => i.ID == bookID.Value).Single();
        ...
        /*[HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("ID,PublisherName,Adress")] Models.Publisher publisher)
        {
            if (id != publisher.ID)
            {
                return NotFound();
